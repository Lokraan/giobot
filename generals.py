...
class Generals(object):
    ...
    def __init__(self, userid, username, mode=None, gameid=None,
        ...
        if(userid):
            ...
            self._send(["set_username", username])
    ...
    def _make_update(self, data):
        ...
        return {
            ...
            'turn': data['turn'],
            'army_grid': [[self._map[2 + x*cols + y]
                          for x in range(rows)]
                          for y in range(cols)],
            'tile_grid': [[self._map[2 + cols*rows + x*cols + y]
                          for x in range(rows)]
                          for y in range(cols)],
